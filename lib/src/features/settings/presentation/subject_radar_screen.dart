...
import 'package:smart_wrong_notebook/src/shared/utils/subject_radar_html.dart';
import 'package:webview_flutter/webview_flutter.dart';

/// 学科能力雷达图预览页：聚合全量题目 → 生成 HTML → WebView 渲染。
///
...
class SubjectRadarScreen extends ConsumerStatefulWidget {
  const SubjectRadarScreen({super.key, this.studentName});

  /// 封面学生姓名；为空时显示下划线占位。
  final String? studentName;

  @override
  ConsumerState<SubjectRadarScreen> createState() =>
      _SubjectRadarScreenState();
}

class _SubjectRadarScreenState extends ConsumerState<SubjectRadarScreen> {
  ...
  Future<void> _init() async {
    try {
      ...
      final html = generateSubjectRadarHtmlSync(
        data,
        studentName: widget.studentName,
      );
      // 写入临时文件，用 loadFile 加载（与 HtmlPreviewScreen 保持一致）。
      final dir = await getTemporaryDirectory();
      final file = File('${dir.path}/subject_radar_preview.html');
  ...
  Future<void> _exportPdfDesktop() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('已在浏览器打开雷达图，可在浏览器中选择「打印 → 另存为 PDF」'),
        ),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  /// PDF 文件名：学科能力雷达图_YYYYMMDD_HHmm.pdf
